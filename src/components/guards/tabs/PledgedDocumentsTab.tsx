...
export default function PledgedDocumentsTab({ documents }: PledgedDocumentsTabProps) {
    const rows = documents as PledgedDocument[]
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600 mt-1">
                                        Received: {doc.receivedDate
                                            ? new Date(doc.receivedDate).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
                                            : "—"}
                                    </p>
                                </div>
                                <span className={`px-2 py-1 text-xs rounded-full font-medium ${getStatusColor(doc.returnStatus || "HELD")}`}>
                                    {doc.returnStatus || "—"}
                                </span>
                            </div>
