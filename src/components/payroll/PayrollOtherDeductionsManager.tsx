...
export default function PayrollOtherDeductionsManager() {
  ...
  const [form, setForm] = useState({ guardId: "", month: "", amount: "" })
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/payroll/other-deductions?${params.toString()}`)
      if (!response.ok) throw new Error("Failed to load records")
      const data = await response.json()
      setRows(data || [])
    } catch (error) {
      console.error(error)
      setNotice({ type: "error", message: "Unable to fetch deduction records." })
    } finally {
      setLoading(false)
    }
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch("/api/payroll/other-deductions", {
        method: "POST",
  ...
  return (
    <div className="space-y-6">
      <SectionTitle title="Other Deductions" subtitle="Backend-connected deduction records." />
      {notice ? <InlineAlert type={notice.type} message={notice.message} /> : null}

      <section className="ui-card p-4 space-y-4">
        <p className="text-sm font-semibold text-[var(--text)]">Add Deduction</p>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="space-y-1">
            ...
            <input className="ui-input" type="number" value={form.amount} onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} />
          </label>
        </div>
        ...
        <table className="w-full min-w-[860px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} className="px-4 py-8 text-center text-sm text-[var(--text-muted)]">No records.</td></tr>
            ) : (
              rows.map((row) => (
