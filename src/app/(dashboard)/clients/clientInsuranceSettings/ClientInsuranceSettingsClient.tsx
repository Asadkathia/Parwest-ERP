...
export default function ClientInsuranceSettingsClient() {
  ...
  // auto-scoped server-side (their session pins regional offices).
  useEffect(() => {
    const url = urlRegionId
      ? `/api/regional-offices?regionId=${encodeURIComponent(urlRegionId)}`
  ...
  useEffect(() => {
    ...
    fetch(`/api/clients?regionalOfficeId=${formRoId}&limit=500${urlRegionId ? `&regionId=${encodeURIComponent(urlRegionId)}` : ""}`)
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    fetch(`/api/clients?regionalOfficeId=${filterRoId}&limit=500${urlRegionId ? `&regionId=${encodeURIComponent(urlRegionId)}` : ""}`)
      .then(r => r.json())
      .then(data => setFilterClients(Array.isArray(data) ? data : (data?.clients ?? [])))
      .catch(() => setFilterClients([]))
  }, [filterRoId, urlRegionId])
