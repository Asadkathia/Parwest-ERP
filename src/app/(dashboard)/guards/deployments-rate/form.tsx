...
export default function DeploymentRatesForm() {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <SectionTitle title="Deployment Rates" subtitle="Configure salary, overtime, extra hours and post allowance rates" />
      ...
      <FilterBar className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select value={regionId} onChange={(e) => setRegionId(e.target.value)} className="ui-select">
          ...
          <div>
            ...
            <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className="ui-select" disabled={!clientId}>
          ...
          <div>
            ...
            <select value={deployAs} onChange={(e) => setDeployAs(e.target.value)} className="ui-select">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-3">
            ...
            <div className="flex gap-4 text-sm mt-2">
              {[["DAY", "Day"], ["NIGHT", "Night"], ["BOTH", "Both"]].map(([value, label]) => (
                <label key={value} className="inline-flex items-center gap-2">
                  <input type="radio" checked={shiftType === value} onChange={() => setShiftType(value)} className="h-4 w-4 accent-[var(--brand)]" />
                  <span>{label}</span>
                </label>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Salary</label><input value={salary} onChange={(e) => setSalary(e.target.value)} className="ui-input" placeholder="Guards salary" type="number" /></div>
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Overtime</label><input value={overtime} onChange={(e) => setOvertime(e.target.value)} className="ui-input" placeholder="Guards overtime pay" type="number" /></div>
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Extra Hours</label><input value={extraHours} onChange={(e) => setExtraHours(e.target.value)} className="ui-input" placeholder="Extra hours salary" type="number" /></div>
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Post Allowance</label><input value={postAllowance} onChange={(e) => setPostAllowance(e.target.value)} className="ui-input" placeholder="Guards post allowance" type="number" /></div>
        </div>
        ...
        <table className="w-full min-w-[1200px]">
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {recentRates.length === 0 ? (
              ...
              recentRates.slice(0, 15).map((rate) => (
