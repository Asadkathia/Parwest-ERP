...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      const assignment = await tx.managerSupervisorAssignment.create({
        data: {
          managerId,
          supervisorId,
          regionalOfficeId,
      ...
      await tx.auditLog.create({
        data: {
          ...
          module: "USERS",
          description: `Assigned supervisor ${supervisorId} to manager ${managerId} (relationship ${assignment.id})`,
        },
      })
