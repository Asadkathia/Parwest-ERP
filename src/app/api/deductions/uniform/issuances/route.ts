...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (trx) => {
      // Idempotency precheck — natural key: (guardId, issuedOn, uniformPlanId).
      // A retried POST for the same guard + issuance date + plan version must
      // return the existing issuance, not spawn a parallel installment schedule.
      // TODO(hardening): replace with @@unique([guardId, issuedOn, uniformPlanId])
      // on UniformIssuance per audit "Top #4 — Non-idempotent issuance triggers"
      // (docs/audits/deductions-pipeline-dead-legacy-conflict-audit.md).
      const existing = await trx.uniformIssuance.findFirst({
        where: { guardId, issuedOn, uniformPlanId: plan.id },
      })
      if (existing) {
        return existing
      }
