...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: "Failed to fetch other deductions records." }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!guard) return NextResponse.json({ message: "Guard not found." }, { status: 404 })
    ...
    return NextResponse.json({ message: "Failed to save other deductions." }, { status: 500 })
  }
}
