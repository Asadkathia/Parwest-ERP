...
> **Keep this file current — always.** After ANY change that affects how the system works (a new invariant, shared helper, schema/migration, security/scoping rule, gotcha, or architectural decision), update CLAUDE.md in the SAME session — don't wait to be asked. Treat it as the AI agent's map of the system: prefer adding the durable *rule* (one line per concept) and a pointer to deeper artifacts over narrating what changed. If an edit makes an existing line wrong, fix that line. A change isn't "done" until CLAUDE.md reflects it.
...
- **Region-less scoping — NEVER gate a client/branch by `client.regionId` (B2)**: branchful `Client.regionId` is NULL, so `managerScopeDenied({ regionId: client.regionId })` **FAILS OPEN** (cross-region leak). Every client-access gate routes through `clientInScope(clientId, scope)` (per-record) / `clientScopeWhere(scope)` (list `where.client`, AND-composed with any topbar `?regionId=`, fails CLOSED) from `src/lib/clients/access.ts` — applied across clients, invoices, client-insurances, and users/cs-relationships routes. **`branches/*` routes** (`/api/branches`, `/api/branches/[id]` PATCH/DELETE/deactivate) scope by the branch's OWN `regionalOffice.regionId` (fail CLOSED on a null-office branch); branch-level audit `targetRegionId` uses that same office region; branch CREATE requires a non-null in-scope office for restricted managers. **Supervisors/Users keep their own `regionId` — leave that scoping alone** (only the client half of cs-relationships was region-less-ified). Migrations adding an FK on a long-dangling column must self-heal orphans (`UPDATE … SET col=NULL WHERE NOT EXISTS …`) BEFORE `ADD CONSTRAINT`, else the validating add aborts the Vercel deploy.
- **Branchless↔branchful conversion invariant (B2)**: "has ≥1 branch ⇒ branchful & region-less". Both branch-create POSTs (`/api/branches`, `/api/clients/[id]/branches`) atomically flip `isBranchless=false` + null the client's `regionId/regionalOfficeId/city/operationalProvinces` inside the create txn. Client PUT gates geo writes on the **effective** post-edit `isBranchless` (not the stored value), nulls geo on convert-to-branchful, and 409s on marking a client branchless while it has branches. Client detail page shows **branch-derived geo** (distinct cities/regions/offices/provinces) for branchful clients; client listings carry NO region/city filters or columns (region filtering stays only on the *branches* list, scoped by `Branch.regionalOffice.region`).
- **Region-less downstream — derive region/province from the DEPLOYMENT's branch office, never the client (B2 sweep, 2026-06-03)**: business logic keyed on guard/deployment/branch-office region is already correct; the fixes were everywhere that read the *client's own* region. **Invoicing**: MANUAL rate scope (`buildLines.ts` + `invoices/auto-fill`) is resolved **per-guard from that guard's own deployment branch** (`guardBranchId = args.branchId ?? agg.latestBranchId`, memoized `resolveBranchScope`), NOT once from the client — else a client-level invoice on a branchful client collapses region/province to null and silently bills GLOBAL. **Payroll reserve %** (`calculate.ts`): chain is `client.reservePct ?? dep.branch.regionalOffice.reservePct ?? client.regionalOffice.reservePct ?? DEFAULT` (branchful clients inherit the office reserve from the deployment branch). **Inventory** cross-office assignment to a CLIENT derives the allowed office set from the client's **branches** (`store-inventory/v2/assignments`), not `client.regionalOfficeId`. **Insights/reports** that scope the Client table use `clientScopeWhere(scope)` (revenue insights a1-a5, client reports active-inactive/enrolled), and `reports/deployments/current` scopes by the deployment's `regionalOffice.regionId` not `guard.regionId`. Remaining medium items tracked in memory `project_unresolved_issues` (2026-06-03 entry).
